...
- **Empty query handling.** `SeedSearcher.start()` now fires `onFound` with a
  ...
  a world with seed 0.
- **SearchScreen widget allocation.** Pre-allocated 16 `StringWidget` lines
  (was 4–5) to prevent silent line drops when queries produce many status
  messages (modifier summaries, biome details, ignored words, etc.).
- **Text centering on MC 1.21.x.** `alignX=0.5` is not accessible via
  reflection (Fabric intermediary remapping). Workaround: `setWidth(textWidth)`
  + `setX(centerX - textWidth/2)` — works on both 1.21.1 and 1.21.11.
